...
| 2026-08-28 | Command completion | Current `Tab` completion edits the composer inline; it lacks the searchable visual command overlay used by Claude TUI. | Medium: commands are discoverable only if the user already knows the slash syntax. | Add a keyboard-navigable command palette overlay with descriptions, argument hints, selection, and Escape-to-close behavior. | deferred |
