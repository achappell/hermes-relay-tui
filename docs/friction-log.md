...
| 2026-08-28 | Homebrew packaging | Homebrew rejects a formula file supplied directly from a project checkout; formulas must be staged inside a tap. | Medium: the Jensen trial could not use a single `brew install` command until a tap existed. | resolved: published the private tap and pinned its Git source to `v0.1.0`; convert to a checksummed archive if the tap becomes public. |
