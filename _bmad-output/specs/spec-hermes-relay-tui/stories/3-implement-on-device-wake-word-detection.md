...
- [ ] Speak "hey jarvis" near the device and confirm a wake event appears in `esphome logs` (or the API) -- attempted 4 times across two builds (with and without a `vad:` block); zero detections and zero audio-pipeline log activity in every attempt. See Implementation Notes — inconclusive, not disproven, closing this story without it rather than continuing to guess blind.
...
- Given a spoken "hey jarvis" near the device, when `micro_wake_word` processes it, then a detection event is observable in logs or the API. **Not met.** Attempted 4 times; unconfirmed, not disproven. Tracked as this story's one open follow-up rather than blocking the rest of the verified work.
