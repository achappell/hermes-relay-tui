...
The private household pilot needs natural access to the household's chosen Hermes relationship from a room, without a phone or terminal, while remaining truthful about which doorway heard, captured, spoke, displayed, or failed. A Puck, room Display, iOS/macOS Client, TUI, and iPad kiosk now make fragmentation the central risk: each surface could invent different identity, turn-state, privacy, and recovery behavior. This work creates one inspectable, household-owned contract before the first-room pilot expands.

## Capabilities

- **CAP-1**
  - **intent:** An authorized Puck or Client can start a turn for a selected Hermes Profile, with the profile fixed before Puck capture.
  - **success:** The dinner journey selects the configured profile before audio capture; iOS and TUI can start their own authorized typed or voice turns; an unavailable or revoked mapping starts no turn.
...
- **CAP-2**
  ...
  - **success:** Tests or a live pilot observe `heard` → `listening` → `transcribing` → `thinking` → `buffering` → `speaking` → `complete` only when known; streamed text and spoken audio represent one response, and completed text with failed audio is marked unavailable rather than falsely speaking.

- **CAP-3**
  - **intent:** A Puck can continue one conversation briefly after a response and recover from transport loss without silently reopening capture or replaying an uncertain turn.
  - **success:** The follow-up window lasts eight seconds; exactly `stop` creates no replacement turn; reconnect creates a fresh Hermes Session and requires a fresh initiation; a dropped active turn is never automatically resent.
...
- **CAP-5**
  - **intent:** Room Displays and the iPad kiosk render shared ambient, active-turn, prompt, and recovery semantics while keeping conversation local to the owning Room.
  - **success:** ESP32-S3/LVGL and browser fixtures accept the same valid states and actions; the selected Room mirrors profile, live transcription, response, and honest failure; other Rooms receive no turn text; prompts are visible but have no touch response action in v1.
...
- Puck audio is transient and home-LAN-only; the Media Server is not a transcript archive, while iOS/TUI may retain deliberate Local History.
...
- Full response text on the Puck's small TFT or default transcript archives on Pucks, Displays, or the Media Server.
- A native iPad Display application, or a multi-room hardware rollout as an MVP prerequisite.
...
- What Media Server audio transport, bounded buffering, and cleanup behavior covers success, cancellation, and disconnect?
...
- What kiosk authentication and offline re-entry behavior does the iPad Display require?
