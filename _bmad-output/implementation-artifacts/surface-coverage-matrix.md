...
|---|---|---|---|---|
| 0 — Standard Hermes compatibility and cross-surface migration | FR-23–FR-27 plus shared migration gate | `hermes-relay-home` with every endpoint owner | [Migration specification](../../hermes-relay-home/_bmad-output/specs/spec-standard-hermes-compatibility-migration/SPEC.md), Home AD-14, TUI AD-24; TUI Story 3 delivery evidence is recorded in [the local spec](spec-3-tui-migrate-the-terminal-client.md) | TUI now has an opt-in direct Standard gateway/session/audio adapter with fork rollback preserved and fake-socket/full-suite evidence. This adapter is not the Home bridge and does not satisfy Home route, Device-credential, opaque-handle, endpoint-envelope, or live route evidence; those remain before any Home-backed default switch. |
