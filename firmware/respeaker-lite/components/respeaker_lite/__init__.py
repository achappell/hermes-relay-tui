...
def download_firmware(config):
    ...
    # Session 17 (story 3) diagnostic: skip external_files.download_content
    # (which does a deferred `import requests` on first call) when a
    # correctly-sized cached copy already exists, to test whether that
    # first-time import is implicated in a reproducible SIGSEGV this
    # session traced to the ESP-IDF toolchain subprocess whenever this
    # firmware: block is present -- isolated by size (200 bytes truncated
    # still crashed) and by presence/absence of this exact block, with
    # swap/memory/disk pressure and even a full OS reboot ruled out as
    # the cause. Revert this skip once confirmed either way.
    if not path.exists():
...
async def to_code(config):
    ...
    if config_fw := config.get(CONF_FIRMWARE):
        ...
        # Session 17 (story 3) patch: upstream builds this as
        # `[HexInt(x) for x in firmware_bin]` (274K+ individual Python
        # objects for this board's firmware, each going through
        # safe_exp/HexIntLiteral) passed to cg.progmem_array. That reliably
        # segfaults in this environment's ESP-IDF toolchain subprocess
        # shortly after (SIGSEGV in _yaml's C extension, per a macOS crash
        # report) -- isolated by testing this exact component with/without
        # the firmware: block, all else identical; never reproduced without
        # it. Root cause not fully confirmed (likely memory-layout/GC
        # pressure from 274K short-lived objects colliding with a later
        # subprocess's shared-library load, not a yaml content bug -- the
        # array is never serialized to YAML), but building the same C++
        # array literal as one pre-formatted raw string instead of 274K
        # Python objects avoids it entirely and produces identical output.
