...
bool RespeakerLite::dfu_get_version_() {
  ...
  // Session 17 (story 3) fix: upstream dereferences this unconditionally,
  // but it's only set when the optional `firmware_version:` text_sensor is
  // configured (respeaker_lite.h:141) -- crashed this exact test with
  // Guru Meditation Error (LoadProhibited) on real hardware, in a boot
  // loop, since our minimal test config doesn't configure that sensor.
  // Matches the existing null-guard pattern already used for mute_state_
  // (get_mic_mute_state_(), line ~352).
