...
// Anti-aliasing lowpass filter applied before 48kHz -> 16kHz decimation
// (Respeaker Lite / Hermes Relay TUI story 3 follow-up). Replaces both the
// original nearest-sample-drop (no filtering at all) and this project's
// earlier 3-tap boxcar attempt (real but weak attenuation -- confirmed by
// spectrogram comparison to still leave wake-word detection broken) with a
// proper 31-tap windowed-sinc (Hamming) FIR, cutoff 7000Hz at 48kHz,
// unity DC gain. Coefficients generated offline; see the story file's
// Implementation Notes for the generating script and the spectrogram
// evidence motivating a steeper filter than the boxcar.
static const size_t FIR_TAPS = 31;
static const float FIR_H[FIR_TAPS] = {
    0.00156741f,  0.00052958f,  -0.00178409f, -0.00444953f, -0.00408776f, 0.00255227f,  0.01299161f,
    0.01693951f,  0.00348824f,  -0.02557294f, -0.04856884f, -0.03371102f, 0.03701281f,  0.14752237f,
    0.24983161f,  0.29147753f,  0.24983161f,  0.14752237f,  0.03701281f,  -0.03371102f, -0.04856884f,
    -0.02557294f, 0.00348824f,  0.01693951f,  0.01299161f,  0.00255227f,  -0.00408776f, -0.00444953f,
    -0.00178409f, 0.00052958f,  0.00156741f,
};
...
void I2SAudioMicrophone::mic_task(void *params) {
  ...
  {  // Ensures the samples vector is freed when the task stops
    ...
    // Per-channel circular history of raw 48kHz samples for the FIR
    // anti-aliasing filter below. Declared here (task lifetime), not
    // inside the while loop, so filter context carries across chunk
    // boundaries instead of restarting cold every ~16ms.
    int32_t fir_history[2][FIR_TAPS] = {{0}};
    size_t fir_write_pos = 0;
    ...
    while (!(xEventGroupGetBits(this_microphone->event_group_) & MicrophoneEventGroupBits::COMMAND_STOP)) {
      if (this_microphone->data_callbacks_.size() > 0) {
        ...
        // 48kHz -> 16kHz decimation with a proper anti-aliasing lowpass
        // filter. The original version of this patch kept the first
        // stereo frame out of every 3 and dropped the other two -- no
        // filtering at all. A first fix used a 3-tap boxcar average --
        // real but weak attenuation. Both were confirmed, by capturing
        // and spectrogram-inspecting real audio (Hermes Relay TUI story
        // 3, firmware/respeaker-lite), to leave wake-word detection
        // broken; the boxcar measurably reduced but did not eliminate
        // the aliasing signature. This applies the 31-tap windowed-sinc
        // FIR defined above (FIR_H, cutoff 7000Hz) before keeping every
        // 3rd sample, run per-channel over a circular history buffer so
        // filter context survives chunk boundaries.
        std::vector<uint8_t> filtered_samples;
        size_t bytes_per_frame = 8;  // stereo, 32-bit, at 48kHz (pre-decimation)
        size_t total_frames = samples.size() / bytes_per_frame;
        filtered_samples.reserve((total_frames / 3 + 1) * bytes_per_frame);

        for (size_t f = 0; f < total_frames; ++f) {
          const uint8_t *frame = samples.data() + f * bytes_per_frame;
          for (int ch = 0; ch < 2; ++ch) {
            size_t off = ch * 4;
            int32_t s = (int32_t) (frame[off] | (frame[off + 1] << 8) | (frame[off + 2] << 16) |
                                    (frame[off + 3] << 24));
            fir_history[ch][fir_write_pos % FIR_TAPS] = s;
          }
          fir_write_pos++;
          total_frames_seen++;

          if (total_frames_seen % 3 == 0) {
            uint8_t out_frame[8];
            for (int ch = 0; ch < 2; ++ch) {
              double acc = 0.0;
              for (size_t t = 0; t < FIR_TAPS; ++t) {
                size_t idx = (fir_write_pos + t) % FIR_TAPS;  // oldest..newest; filter is symmetric so order doesn't matter
                acc += (double) FIR_H[t] * (double) fir_history[ch][idx];
              }
              if (acc > 2147483647.0)
                acc = 2147483647.0;
              if (acc < -2147483648.0)
                acc = -2147483648.0;
              int32_t sample = (int32_t) acc;
              size_t off = ch * 4;
              out_frame[off] = (uint8_t) (sample & 0xFF);
              out_frame[off + 1] = (uint8_t) ((sample >> 8) & 0xFF);
              out_frame[off + 2] = (uint8_t) ((sample >> 16) & 0xFF);
              out_frame[off + 3] = (uint8_t) ((sample >> 24) & 0xFF);
            }
            filtered_samples.insert(filtered_samples.end(), out_frame, out_frame + 8);
          }
        }

        this_microphone->data_callbacks_.call(filtered_samples);
      } else {
