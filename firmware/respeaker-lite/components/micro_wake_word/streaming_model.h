...
class StreamingModel {
 public:
  ...
  // TEMPORARY diagnostic (story 3 follow-up): a stable, human-readable name
  // for the mww_prob log, since raw pointer addresses aren't identifiable
  // across models without cross-referencing separately.
  virtual const char *debug_name() const { return "?"; }
 ...
 protected:
  ...
  // TEMPORARY diagnostic (see perform_streaming_inference): per-instance
  // throttle so each of the 3 models (hey_jarvis, stop, vad) gets its own
  // ~1/s probability log instead of one shared throttle starving the
  // other two.
  uint32_t last_prob_log_ms_{0};
...
class WakeWordModel final : public StreamingModel {
 public:
  ...
  const char *debug_name() const override { return this->id_.c_str(); }
...
class VADModel final : public StreamingModel {
 public:
  ...
  const char *debug_name() const override { return "vad"; }
};
