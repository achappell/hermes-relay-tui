      ...
      while (!(xEventGroupGetBits(this_mww->event_group_) & (COMMAND_STOP | ERROR_BITS))) {
        ...
        while (audio_source->available() >= sizeof(int16_t)) {
          ...
          if (feature_generated) {
            // TEMPORARY diagnostic (story 3 follow-up session 8): both
            // hey_jarvis and okay_nabu flatline near-zero probability
            // while VAD (fed this exact same features_buffer) responds
            // normally -- ruling out the audio pipeline, model files, and
            // tensor shape/arena sizing (all separately checked). This
            // logs the actual int8 feature values reaching every model,
            // which no prior session ever looked at, to see whether their
            // numeric range/distribution looks like real speech-derived
            // features at all.
            static uint32_t last_feat_log_ms = 0;
            uint32_t feat_now = millis();
            if (feat_now - last_feat_log_ms > 100) {
              int8_t fmin = features_buffer[0], fmax = features_buffer[0];
              int32_t fsum = 0;
              for (size_t i = 0; i < PREPROCESSOR_FEATURE_SIZE; ++i) {
                fmin = std::min(fmin, features_buffer[i]);
                fmax = std::max(fmax, features_buffer[i]);
                fsum += features_buffer[i];
              }
              char feat_str[PREPROCESSOR_FEATURE_SIZE * 5 + 1];
              size_t pos = 0;
              for (size_t i = 0; i < PREPROCESSOR_FEATURE_SIZE; ++i) {
...
bool MicroWakeWord::generate_features_(const int16_t *audio_buffer, size_t samples_available,
  ...
  // TEMPORARY diagnostic (story 3 follow-up session 8): the quantized
  // features reaching every model are pinned near -128 (the floor) almost
  // always, despite confirmed huge raw mic amplitude -- this measures the
  // actual int16 PCM magnitude this component itself consumes (post
  // MicrophoneSource conversion), to see whether that conversion is
  // delivering the healthy-magnitude samples the math predicts, or
  // something much quieter than expected.
  {
    static uint32_t last_pcm_log_ms = 0;
    uint32_t pcm_now = millis();
    if (pcm_now - last_pcm_log_ms > 1000 && samples_available > 0) {
      int16_t pmin = audio_buffer[0], pmax = audio_buffer[0];
