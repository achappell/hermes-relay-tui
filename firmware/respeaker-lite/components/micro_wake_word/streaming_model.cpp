...
bool StreamingModel::load_model_() {
  ...
  // TEMPORARY diagnostic (story 3 follow-up): compare the actual loaded
  // tensor shape/stride/arena size across model types (WakeWordModel vs
  // VADModel), since both tested wake-word models flatlined near-zero
  // probability while VAD responded normally on identical shared input --
  // ruling out per-model file corruption and pointing at something
  // structurally different between the two model classes.
  {
    TfLiteTensor *diag_input = this->interpreter_->input(0);
    ESP_LOGI("mww_shape", "model %s loaded: stride=%d feature_size=%d arena_size=%zu sliding_window=%zu",
             this->debug_name(), (int) diag_input->dims->data[1], (int) diag_input->dims->data[2],
             this->tensor_arena_size_, this->sliding_window_size_);
  }
...
bool StreamingModel::perform_streaming_inference(const int8_t features[PREPROCESSOR_FEATURE_SIZE]) {
  ...
  if (this->loaded_) {
    ...
    if (this->current_stride_step_ >= stride) {
      ...
      // TEMPORARY diagnostic (Hermes Relay TUI story 3 follow-up): the
      // component never logs a raw probability value, only a binary
      // "detected"/"not detected" outcome once every 10ms-ish inference
      // step -- across every prior session (extreme volume, two
      // different anti-aliasing filters) we never saw a single
      // "predicts... but VAD doesn't" or "Detected" log line, which only
      // tells us the threshold was never crossed, not how close probability
