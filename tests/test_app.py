...
async def test_submitting_input_sends_a_turn_and_clears_input():
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause()
        assert session.sent_turns == [("hello hermes", "local")]
        assert input_widget.value == ""
        assert "you> hello hermes" in transcript_of(app)
...
async def test_second_turn_is_refused_while_one_is_in_flight():
    # Two concurrent turns would both call ws.recv() and raise
    # websockets.ConcurrencyError; the second must be refused instead.
    gate = asyncio.Event()
