...
2. Run the script, passing every epic file:
   ...
   `{date}` must be `MM-DD-YYYY HH:MM` — the format the staleness check parses. The script owns parsing (`## Epic N:` / `### Story N.M: Title` → kebab-case keys, plus rows under `### Surface-specific Epic N story map` → `N-surface-number-slug`; fenced code blocks ignored), ordering (epic, its stories, its retrospective), merging with any existing file (preserve advanced statuses, never downgrade; legacy v6 values like `drafted`/`contexted` are normalized to their modern meaning, never reset; `action_items`, custom keys, and user comments carried through; `project_key`/`tracking_system`/`story_location` kept from the existing file unless overridden by flag), story-file detection (a story file on disk floors its status at `ready-for-dev`), atomic writes, and post-write validation. When an epic has a surface map, generic numeric headings are acceptance templates; they are retained only when a matching implementation artifact exists as a local alias. It prints a JSON report. Add `--dry-run` to preview — the report's `in_sync`, `new_entries`, `dropped_orphans`, `illegal`, and `legacy_mapped` fields answer "is tracking in sync?" without writing.
...
3. Read the JSON report and act on it — this is where judgment re-enters:
   ...
   - `dropped_orphans` are entries that existed in the old status file but match nothing in the epics — usually renames, or generic template rows replaced by surface identities. Each carries its old status; reconcile with the user, then transplant by rerunning with `--set <new-key>=<old-status>` when the old entry represents real delivery evidence.
