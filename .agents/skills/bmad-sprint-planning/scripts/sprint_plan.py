...
SURFACE_HEADING_RE = re.compile(
    r"^#{1,4}\s*Surface-specific\s+Epic\s+(\d+)\s+story\s+map\b",
    re.IGNORECASE,
)
SURFACE_ROW_RE = re.compile(
    r"^\|\s*`?((?:(?:\d+-)?[A-Za-z]+-\d+))`?\s*\|\s*([^|]+?)\s*\|\s*(.*?)\s*\|\s*$"
)
SURFACE_KEY_RE = re.compile(r"^(\d+)-([a-z]+)-(\d+)-.+$", re.IGNORECASE)
...
# split-story keys like 2-6a-...; surface rows use epic-surface-number-slug
# (for example 1-p-1-authorized-wake-and-capture). The local BMad runtime reads
...
def _story_sort_key(key):
    m = SURFACE_KEY_RE.match(key)
    if m:
        return (int(m.group(1)), int(m.group(3)), m.group(2).lower(), key)
    m = STORY_KEY_RE.match(key)
    if not m:
...
def _surface_story_key(raw_id, epic_num, scope):
    """Return a stable tracker key for a surface-map story row.

    Epic 1 uses compact IDs such as ``P-1`` while later maps include the epic
    number (``2-P-1``). The tracker key always carries the epic number so the
    existing status vocabulary can associate it with its parent epic.
    """
    match = re.fullmatch(r"(?:(\d+)-)?([A-Za-z]+)-(\d+)", raw_id.strip())
    if not match:
        return None
    prefix, surface, number = match.groups()
    if prefix is not None and int(prefix) != epic_num:
        return None
    # Keep alias prose out of the key slug while retaining the row's exact
    # surface identity in the leading segments.
    title = scope.strip().split(". The historical numeric artifact", 1)[0].rstrip(".")
    return f"{epic_num}-{surface.lower()}-{int(number)}-{_slug(title)}"


def parse_epics(paths, stories_dir=None):
    """Return (entries, warnings), including surface-map delivery stories.

    Once an epic has a surface-specific map, its generic numeric Story
    headings are acceptance templates. A numeric story is retained only when
    its owning implementation artifact exists (the current TUI 1.1–1.4
    records are the deliberate exception/alias). This keeps the generated
    tracker from growing a duplicate backlog alongside the surface queue.
    """
    epics = {}  # epic_num -> [story keys in order]
    surface_epics = {}  # epic_num -> [(surface key, surface, number)]
    warnings = []
    story_files = set()
    if stories_dir and Path(stories_dir).is_dir():
        story_files = {p.name for p in Path(stories_dir).glob("*.md")}

    def has_story_artifact(story_key):
        return (
            f"{story_key}.md" in story_files
            or f"spec-{story_key}.md" in story_files
        )
    for path in paths:
        try:
        ...
        for lineno, line in enumerate(lines, 1):
            ...
            if in_fence:
                continue
            surface_heading = SURFACE_HEADING_RE.match(line)
            if surface_heading:
                surface_epic = int(surface_heading.group(1))
                epics.setdefault(surface_epic, [])
                surface_epics.setdefault(surface_epic, [])
                continue
            if line.startswith("#") and not SURFACE_HEADING_RE.match(line):
                surface_epic = None
            epic_m = EPIC_RE.match(line)
            if epic_m:
                epics.setdefault(int(epic_m.group(1)), [])
                continue
            if surface_epic is not None:
                surface_row = SURFACE_ROW_RE.match(line)
                if surface_row:
                    raw_id, _surface, scope = surface_row.groups()
                    key = _surface_story_key(raw_id, surface_epic, scope)
                    if key is None:
                        warnings.append(
                            f"invalid surface story key '{raw_id}' at {path}:{lineno}"
                        )
                    else:
                        parts = re.fullmatch(
                            r"(?:(\d+)-)?([A-Za-z]+)-(\d+)", raw_id.strip()
                        )
                        assert parts is not None
                        surface_epics[surface_epic].append(
                            (key, parts.group(2).lower(), int(parts.group(3)))
                        )
                    continue
            ...
            if story_m:
                ...
                if key in stories:
                    ...
                    stories.append(key)
                continue
            if re.match(r"^#{1,4}\s*(?:Epic List|.*Epic Breakdown)\s*$", line, re.IGNORECASE):
                continue
    ...
    for epic_num in sorted(epics):
        ...
        surface_rows = surface_epics.get(epic_num, [])
        if surface_rows:
            # Generic numeric headings remain formal only when their matching
            # story artifact exists. This retains the local TUI records while
            # treating the other numeric headings as acceptance templates.
            for story_key in epics[epic_num]:
                if has_story_artifact(story_key):
                    entries.append((story_key, "story", epic_num))
            numeric_aliases = {
                int(match.group(2))
                for numeric_key in epics[epic_num]
                if has_story_artifact(numeric_key)
                and (match := re.match(r"^(\d+)-(\d+)[a-z]?-.+$", numeric_key))
                and int(match.group(1)) == epic_num
            }
            for story_key, surface, number in surface_rows:
                # T-1..T-4 are represented by the stable numeric TUI aliases
                # above; adding both would count one delivery story twice.
                if surface == "t" and number in numeric_aliases:
                    continue
                entries.append((story_key, "story", epic_num))
        else:
            for story_key in epics[epic_num]:
...
def build_status(entries, existing_data, stories_dir, warnings):
    ...
    for key, kind, _epic_num in entries:
        ...
        computed = default
        if kind == "story" and has_story_artifact(key):
...
def cmd_generate(args):
    entries, warnings = parse_epics(args.epic_file, args.stories_dir)
    if not entries:
    ...
    if not isinstance(data, dict):
        ...
        if dev is not None and not isinstance(dev, dict):
            ...
            for key, raw in dev.items():
                ...
                if parsed is None:
                    problems.append(
                        f"unrecognized key '{key}' (expected epic-N, N-M-slug, "
                        "epic-surface-N-slug, or epic-N-retrospective)"
                    )
                    continue
                kind, _ = parsed
                status, was_legacy = _normalize(raw)
