...
SURFACE_EPICS_FIXTURE = """\
## Epic 1: Conversation

### Surface-specific Epic 1 story map

| Surface story key | Surface | Scope |
|---|---|---|
| `I-1` | iOS Client | Authorized initiation. |
| `T-1` | TUI | Authorized initiation. |
| `E-2` | ESP32 Touch Display | Honest phases and response rendering. |

### Story 1.1: Authorized Initiation

## Epic 2: Room State

### Surface-specific Epic 2 story map

| Surface story key | Surface | Scope |
|---|---|---|
| `2-P-1` | ReSpeaker Puck | Expose local status. |
"""
...
def test_surface_maps_become_delivery_stories_and_tui_alias_is_not_duplicated(tmp_path, capsys):
    status_file = run_generate(
        tmp_path,
        epics_text=SURFACE_EPICS_FIXTURE,
        stories=("1-1-authorized-initiation",),
    )
    result = out_json(capsys)
    data = load(status_file)
    keys = list(data["development_status"].keys())
    assert keys == [
        "epic-1",
        "1-1-authorized-initiation",
        "1-i-1-authorized-initiation",
        "1-e-2-honest-phases-and-response-rendering",
        "epic-1-retrospective",
        "epic-2",
        "2-p-1-expose-local-status",
        "epic-2-retrospective",
    ]
    assert result["stories"] == 4
    assert "1-t-1-authorized-initiation" not in keys
    assert data["development_status"]["1-1-authorized-initiation"] == "ready-for-dev"
    assert data["development_status"]["1-i-1-authorized-initiation"] == "backlog"
